...
export const fetchLandingImages = async () => {
  ...
  const data = await client.fetch(`*[_type == "landing"][1]{
      ...,
      images[] {
              asset->{
                  ...,
...
export const fetchLandingProducts = async () => {
  ...
  const data = await client.fetch(`
  *[_type == "product" && drop == "Tuko" && !sold] | order(_updatedAt asc) [0..7] {
        ...,
				images[] {
					asset->{
						...,
...
export const fetchCatalogProducts = async (searchParams,interval) => {
  ...
  const data = await client.fetch(`
  *[_type == "product" ${query}] | order(price desc) | order(_updatedAt asc) | order(sold==true) [${interval * 10}...${(interval + 1) * 10}] {
        ...,
				images[] {
					asset->{
						...,
