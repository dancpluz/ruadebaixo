...
export const fetchLookBook = async (index=1) => {
  const data = await client.fetch(`*[_type == "photos"] | order(date desc)[${index}]{
      ...,
      images[] {
              asset->{
                  url,
...
export async function countLookBook() {
  const count = await client.fetch(`
  count(*[_type == "photos"])`,{ cache: 'no-store' });
...
export const fetchLandingProducts = async () => {
  ...
  *[_type == "product" && drop == "Birosca"] | order(_updatedAt desc) [0..7] {
        ...,
				images[] {
					asset->{
						...,
...
export const fetchLook = async (slug) => {
  // Retorna as informações do produto
  const data = await client.fetch(`
