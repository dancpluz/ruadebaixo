...
export const fetchLandingProducts = cache(async () => {
  const data = await client.fetch(groq`*[_type == 'product' && drop == 'Valendo uma Coca'][0..5]{name, slug, type, images, price, drop, quality, tags, wears, size, details, discount, sold, ordered}`);
