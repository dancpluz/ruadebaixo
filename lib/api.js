...
export const fetchLandingProducts = async () => {
  ...
  const data = await client.fetch(`
  *[_type == "product" && drop == "+90"][0..7] | order(_createdAt asc) {
        ...,
				images[] {
					asset->{
						...,
...
export const fetchCatalogProducts = async (searchParams) => {
  // Retorna os produtos do catálogo de acordo com os parâmetros de query
  const params = new URLSearchParams(searchParams)
  const translate = {'tipo': 'type', 'tamanho': 'size', 'categoria': 'category', 'qualidade': 'quality', 'drop':'drop'};
  let queryArray = [];

  for (const [key, value] of params.entries()) {
    queryArray.push(`${translate[key]} in ${JSON.stringify(value.split(','))}`)
  }
  
  const queryString = queryArray.length === 1 ? queryArray[0] : queryArray.join(' && ');

  const data = await client.fetch(`
  *[_type == "product" ${queryArray.length == 0 ? '' : '&& ' + queryString} ] | order(_createdAt desc) | order(sold==true) {
        ...,
				images[] {
					asset->{
						...,
