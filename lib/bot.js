...
function getDateNow(newFormat=false) {
  const options = {
    year: 'numeric',
    month: '2-digit',
    day: '2-digit',
...
export async function sendMessageSticker(json) {
  const date = getDateNow(true);
  const { nome, insta, anonimo, feedback } = json;

  const message = `*NOVO ADESIVO*
👤 ${nome}
🤳 ${insta}${anonimo ? '\n⚠️ *Optou por ficar anônimo*' : '' }${feedback ? `\n🗣️ Feedback: ${feedback}` : ''}
🕑 Enviado em ${date}`
