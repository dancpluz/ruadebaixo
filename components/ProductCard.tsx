...
import { cn, formatToBRL, applyDiscount } from "@/lib/utils";
...
export default function ProductCard({ product }: { product: Produto }) {
  ...
  const hoverAnim = 'relative flex after:absolute after:bg-foreground after:bottom-0 after:h-[1px] after:w-full after:origin-bottom-right after:scale-x-0 hover:after:origin-bottom-left hover:after:scale-x-100 after:transition-transform after:ease-in-out after:duration-300'
    ...
    <Card className='border-0 relative'>
      ...
      <Link href={`/catalogo/${slug}`}>
        <CardFooter className='justify-between p-0 pt-2'>
          ...
          {desconto ?
            <CardDescription className={cn('text-xl gap-2', hoverAnim)}><strong className='font-normal opacity-50 line-through'>{formatToBRL(valor)}</strong>{formatToBRL(applyDiscount(valor, desconto))}</CardDescription>
            :
